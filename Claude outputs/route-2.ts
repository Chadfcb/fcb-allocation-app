...
//
// Raised to 300s / 50 rounds on 2026-09-23, same as
// app/api/ernie/chat/route.ts (see the comments there): Vercel's Hobby
// limit with Fluid compute is 5 minutes, and Ernie's real limit is now
// time (lib/ernie/replyBudget.ts), with 50 rounds only as a runaway guard.
...
The message that was just posted, from ${senderName}${
        ...
        for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
          // Last round = hit the runaway guard, or out of time for this
          // reply (lib/ernie/replyBudget.ts) — forced plain-text wrap-up.
            ...
            for (const block of content) {
              ...
              try {
                result = isReplyOutOfTime(replyStartedAt)
                  ...
                  : await runErnieTool(supabase, block.name, block.input ?? {}, role, sections, noConversationId, isSuperAdmin, user.id, requestId, projectId);
              } catch (toolErr) {
                result = { error: toolErr instanceof Error ? toolErr.message : "Tool lookup failed" };
              }
              if (block.name === "browse_website") {
                await logBrowseStep(supabase, user.id, noConversationId, block.input, result);
              }
              if (
