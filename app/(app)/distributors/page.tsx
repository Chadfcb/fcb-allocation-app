...
export default function DistributorsPage() {
  ...
  if (!week) {
    return <p className="text-sm text-neutral-500">No week has been started yet.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold text-neutral-900">Distributor Data</h1>
        ...
        <div className="flex items-end gap-3">
          ...
          <label className="text-sm text-neutral-600">
            ...
            <input
              ...
              className="ml-2 w-16 rounded-md border border-neutral-300 px-2 py-1.5 text-sm"
            />
          </label>
          <button
        ...
        <table className="min-w-full divide-y divide-neutral-200 text-sm">
          <thead className="bg-neutral-50 text-xs uppercase tracking-wide text-neutral-500">
            <tr>
              ...
              <th className="px-2 py-2 text-right">On Hand (Distributor)</th>
              <th className="px-2 py-2 text-right">Rate of Sale / wk</th>
              <th className="px-2 py-2 text-right">Weeks of Supply</th>
              <th className="px-2 py-2 text-left">Source</th>
              <th className="px-2 py-2 text-right font-semibold">Suggested Order</th>
            </tr>
          </thead>
            ...
            {products.map((p) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-neutral-50">
                  <td className="px-3 py-1.5 font-medium text-neutral-800">{p.name}</td>
                  ...
                  <td className="px-2 py-1.5 text-left text-neutral-500 uppercase text-xs">
                    {data[p.id]?.source ?? "—"}
                  </td>
                  <td
                    className={`px-2 py-1.5 text-right font-semibold ${
                      suggestion > 0 ? "text-amber-700" : "text-neutral-400"
                    }`}
                  >
                    {suggestion}
                  </td>
