...
// redirect guard lives in page.tsx.
...
import {
  CHAIN_EVENT_TYPE_LABELS,
  CHAIN_EVENT_TYPE_OPTIONS,
  buildChainMonthGrid,
...
const CHAIN_EVENT_COLOR_PALETTE = [
  "#4d9de0",
  "#d99a3d",
  "#e05c5c",
  "#6abc46",
...
export default function ChainCalendarPageClient() {
  ...
  const [formColor, setFormColor] = useState(CHAIN_EVENT_COLOR_PALETTE[0]);
  ...
  const load = useCallback(async () => {
    ...
    (materialsRes.data as ChainEventMaterial[] | null)?.forEach((m) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("chain-events-live")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "chain_events" },
        load,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "chain_event_materials" },
        load,
      )
      .on(
        "postgres_changes",
  ...
  function openAddModal(startDate?: string) {
    ...
    setFormLocation("");
    setFormRep("");
    setFormColor(CHAIN_EVENT_COLOR_PALETTE[0]);
    setFormNotes("");
    setFormError(null);
    setModalOpen(true);
  }
  function openEditModal(ev: ChainEvent) {
    setEditingId(ev.id);
    ...
    setFormRep(ev.rep ?? "");
    setFormColor(ev.color ?? CHAIN_EVENT_COLOR_PALETTE[0]);
  ...
  async function handleSaveEvent() {
    ...
    if (editingId) {
      ...
      const created = data as ChainEvent;
      await logChange(supabase, {
        weekId: null,
        tableName: "chain_events",
        recordId: created.id,
  ...
  async function handleDeleteEvent(ev: ChainEvent) {
    if (!userId) return;
    if (
      !window.confirm(
      ...
      return;
    const { error } = await supabase.from("chain_events").delete().eq("id", ev.id);
    if (error) return;
    await logChange(supabase, {
      weekId: null,
      tableName: "chain_events",
  ...
  async function handleUploadMaterials(ev: ChainEvent, fileList: FileList) {
    ...
    for (const file of Array.from(fileList)) {
      ...
      await supabase.from("chain_event_materials").insert({
        event_id: ev.id,
        file_name: file.name,
        storage_path: path,
    ...
    await logChange(supabase, {
      weekId: null,
      tableName: "chain_event_materials",
      recordId: ev.id,
      fieldName: "materials",
      oldValue: null,
  ...
  async function handleDeleteMaterial(m: ChainEventMaterial, eventTitle: string) {
    if (!userId) return;
    if (!window.confirm(`Remove "${m.file_name}" from this event?`)) return;
    await supabase.storage
      .from(EVENT_MATERIALS_BUCKET)
      .remove([m.storage_path]);
    await supabase.from("chain_event_materials").delete().eq("id", m.id);
    await logChange(supabase, {
      weekId: null,
      tableName: "chain_event_materials",
      recordId: m.id,
  ...
  async function handleAttachToEvent(ev: ChainEvent) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Chain/retail account activity — demos, resets, ads, and displays.
          </p>
        </div>
                ...
                {grid.map((day) => (
                  ...
                  >
                    ...
                    {day.events.slice(0, 3).map((ev) => {
                      const color = ev.color || CHAIN_EVENT_COLOR;
                      return (
                        <button
                          key={ev.id}
                          type="button"
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Safeway Demo Weekend"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={formType}
                    onChange={(e) => setFormType(e.target.value as ChainEventType)}
                    className="w-full rounded-md border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-sm text-neutral-100"
                  >
                    {CHAIN_EVENT_TYPE_OPTIONS.map((opt) => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CHAIN_EVENT_COLOR_PALETTE.map((c) => (
                    <button
                      key={c}
                      type="button"
                ...
                .map((ev) => (
                  ...
                  >
                    <span
                      ...
                      style={{ background: ev.color || CHAIN_EVENT_COLOR }}
                    />
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div>
        ...
        <div className="text-neutral-200">{CHAIN_EVENT_TYPE_LABELS[event.type]}</div>
      </div>
      {event.notes && (
        <div>
          <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-neutral-500">
