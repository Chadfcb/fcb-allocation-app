...
// (posters, table tents, stickers, distributor strips). No brand/size
// nesting like Labels — this is one shared pool, not per-brand artwork.
// Added 2026-09-05, per Chad: "these items need to go in there with
// Previews and download abilities, they need to look just like how we did
// the labels." Copied from PosLabelFilesClient.tsx's load/realtime/
// preview/download/audit-log conventions almost verbatim, with the
// brand/size scoping removed.

import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { createClient } from "@/lib/supabase/client";
import { logChange } from "@/lib/audit";
import type { PosFootballFile } from "@/lib/types/db";
import {
  fileIcon,
  formatBytes,
  isImageFile,
...
import { POS_FOOTBALL_FILES_BUCKET } from "@/lib/posFootball";

export default function FootballPosFilesClient() {
  const supabase = useMemo(() => createClient(), []);

  const [files, setFiles] = useState<PosFootballFile[]>([]);
  const [userId, setUserId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [uploading, setUploading] = useState(false);
  const [signedUrls, setSignedUrls] = useState<Record<string, string>>({});
  const [previewFile, setPreviewFile] = useState<PosFootballFile | null>(null);
  const [downloadingPath, setDownloadingPath] = useState<string | null>(null);

  const fileInputRef = useRef<HTMLInputElement | null>(null);

  function previewSourcePath(f: PosFootballFile): string | null {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("pos_football_files")
      .select("*")
      .order("uploaded_at", { ascending: false });

    setFiles((data as PosFootballFile[]) ?? []);
    setLoading(false);
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("pos-football-files")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "pos_football_files" },
        () => load(),
      )
      .subscribe();
    return () => {
      ...
      const entries = await Promise.all(
        missing.map(async (p) => {
          const { data } = await supabase.storage
            .from(POS_FOOTBALL_FILES_BUCKET)
  ...
  async function handleUpload(fileList: FileList) {
    ...
    for (const file of Array.from(fileList)) {
      const path = storageFileName(file.name);
      const { error: uploadError } = await supabase.storage
        .from(POS_FOOTBALL_FILES_BUCKET)
        .upload(path, file);
      if (uploadError) continue;
      await supabase.from("pos_football_files").insert({
        file_name: file.name,
        storage_path: path,
    ...
    await logChange(supabase, {
      weekId: null,
      tableName: "pos_football_files",
      recordId: userId,
      fieldName: "football_pos",
      oldValue: null,
  ...
  async function handleDelete(f: PosFootballFile) {
    if (!userId) return;
    ...
    await supabase.from("pos_football_files").delete().eq("id", f.id);
    await logChange(supabase, {
      weekId: null,
      tableName: "pos_football_files",
      recordId: f.id,
  ...
  async function handleDownload(f: PosFootballFile) {
    if (!f.storage_path) {
  ...
  async function openPreview(f: PosFootballFile) {
    setPreviewFile(f);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-neutral-100">Football POS</h1>
      ...
      {files.length === 0 ? (
        <p className="py-10 text-center text-sm text-neutral-500">
          No Football POS files yet. Upload some to get started.
        </p>
      ) : (
