...
import {
  SOCIAL_MEDIA_EVENT_TYPE_LABELS,
  SOCIAL_MEDIA_EVENT_TYPE_OPTIONS,
...
export default function SocialMediaCalendarPageClient() {
  ...
  const load = useCallback(async () => {
    ...
    const [eventsRes, materialsRes, libraryRes] = await Promise.all([
      ...
      supabase
        .from("social_media_event_materials")
        .select("*")
    ...
    (materialsRes.data as SocialMediaEventMaterial[] | null)?.forEach((m) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("social-media-events-live")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "social_media_events" },
        load,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "social_media_event_materials" },
        load,
      )
      .on(
        "postgres_changes",
  ...
  function openEditModal(ev: SocialMediaEvent) {
    setEditingId(ev.id);
  ...
  async function handleSaveEvent() {
    ...
    if (editingId) {
      ...
      const created = data as SocialMediaEvent;
      await logChange(supabase, {
        weekId: null,
        tableName: "social_media_events",
        recordId: created.id,
  ...
  async function handleDeleteEvent(ev: SocialMediaEvent) {
    if (!userId) return;
    if (
      !window.confirm(
      ...
      return;
    const { error } = await supabase.from("social_media_events").delete().eq("id", ev.id);
    if (error) return;
    await logChange(supabase, {
      weekId: null,
      tableName: "social_media_events",
  ...
  async function handleUploadMaterials(ev: SocialMediaEvent, fileList: FileList) {
    ...
    for (const file of Array.from(fileList)) {
      ...
      await supabase.from("social_media_event_materials").insert({
        event_id: ev.id,
        file_name: file.name,
        storage_path: path,
    ...
    await logChange(supabase, {
      weekId: null,
      tableName: "social_media_event_materials",
      recordId: ev.id,
      fieldName: "materials",
      oldValue: null,
  ...
  async function handleDeleteMaterial(m: SocialMediaEventMaterial, eventTitle: string) {
    if (!userId) return;
    if (!window.confirm(`Remove "${m.file_name}" from this event?`)) return;
    await supabase.storage
      .from(EVENT_MATERIALS_BUCKET)
      .remove([m.storage_path]);
    await supabase.from("social_media_event_materials").delete().eq("id", m.id);
    await logChange(supabase, {
      weekId: null,
      tableName: "social_media_event_materials",
      recordId: m.id,
  ...
  async function handleAttachToEvent(ev: SocialMediaEvent) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Planning social media content — posts, campaigns, stories, and promotions.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Fall Release Instagram Campaign"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={formType}
                    onChange={(e) => setFormType(e.target.value as SocialMediaEventType)}
                    className="w-full rounded-md border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-sm text-neutral-100"
                  >
                    {SOCIAL_MEDIA_EVENT_TYPE_OPTIONS.map((opt) => (
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div>
        ...
        <div className="text-neutral-200">{SOCIAL_MEDIA_EVENT_TYPE_LABELS[event.type]}</div>
      </div>
      {event.notes && (
        <div>
          <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-neutral-500">
