      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-800 bg-neutral-950">
        <table className="min-w-full divide-y divide-neutral-900 text-sm">
          ...
          <tbody className="divide-y divide-neutral-900">
            {entries.map((e) => (
              <tr key={e.id} className={`hover:bg-neutral-900/60 ${e.reverted ? "opacity-50" : ""}`}>
                ...
                <td className="px-3 py-1.5 text-neutral-400">{e.table_name}</td>
                <td className="px-3 py-1.5 text-neutral-400">{e.field_name}</td>
                <td className="px-3 py-1.5 text-neutral-500">{e.old_value ?? "—"}</td>
                <td className="px-3 py-1.5 font-medium text-neutral-200">{e.new_value ?? "—"}</td>
