...
Any table above with a storage_path column (event_materials, pos_library, pos_label_files, ernie_reference_documents today — there may be more as the app grows) is describing a real file, not just data. Querying one of those only tells you the file EXISTS — to actually hand it to the user as a download, call get_file_for_download with that row's storage_path and its bucket (event-materials for event_materials/pos_library, pos-label-files for pos_label_files, reference-docs for ernie_reference_documents). Whenever someone asks you to pull up, send them, or let them download a specific file — not just tell them about it — that's the tool to reach for.`,
    input_schema: {
      type: "object" as const,
      properties: {
        query: {
