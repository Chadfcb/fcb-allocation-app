...
-- existing policy here that opens up reading OTHER people's `profiles` rows
-- app-wide, and this migration deliberately doesn't add one. Each person can
-- always read their OWN profile (used everywhere else in the app already),
-- so the app fills in display_name from that at the moment a score is
-- submitted — the leaderboard read then never has to touch `profiles` at
-- all.
